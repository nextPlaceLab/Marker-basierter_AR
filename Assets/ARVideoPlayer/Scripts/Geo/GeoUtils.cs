...
namespace UtyMap.Unity.Utils
{
    ...
    public static class GeoUtils
    {
        ...
        /// KH: added function
        public static GeoCoordinate ToGeoCoordinate(GeoCoordinate relativeNullPoint, Vector3 mapPoint)
        {
            return ToGeoCoordinate(relativeNullPoint, mapPoint.x, mapPoint.z);
        }

        public static BoundingBox BoundingBox(GeoCoordinate geoPos, double size)
        {
            var g1 = new GeoCoordinate(geoPos.Latitude - size, geoPos.Longitude - size);
            var g2 = new GeoCoordinate(geoPos.Latitude + size, geoPos.Longitude + size);
            return new BoundingBox(g1, g2);
        }
        ...
        #endregion Coordinate convertion
        ...
        public static double Length(IList<GeoCoordinate> path, bool isClosed = false)
        {
            ...
            if (isClosed)
                l += Distance(path[path.Count - 1], path[0]);
        ...
        public static IEnumerable<QuadKey> GetNeighboursN(QuadKey quadKey, int n)
        {
            int x = quadKey.TileX;
            int y = quadKey.TileY;
            ...
            for (int i = -n; i <= n; i++)
            {
                for (int j = -n; j <= n; j++)
                {
                    yield return new QuadKey(x + i, y + j, quadKey.LevelOfDetail);
                }
            }
        }
